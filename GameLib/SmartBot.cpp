...
std::shared_ptr<GameDecision> SmartBot::getGameDecision(
    std::shared_ptr<GameField const> gameField) const
{
    while (true)
    {
        ...
        auto &playerPosition =
            gameField->getPosition(*gameField->getPlayerPosition(playerData)).getVertex();
        ...
        auto way =
            search.getPath(gameField->getPosition({x, gameField->getHeight() / 2}).getVertex());
        int nodex;
        int nodey;
